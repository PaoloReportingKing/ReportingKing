    ...
    if (chartconfigobj != undefined) {
        ...
        if (chartconfigobj.chartXAxis.length > 0) {
            ...
            $.each(chartconfigobj.chartXAxisLabels, function (i, xaxis) {
                if (chartconfigobj.TableLocked && i > 5) {
                    return false;
                }
                var tds = "";
                var seriesobj = [];
        ...
        if (chartconfigobj.chartXAxis.length > 0) {
            ...
            $.each(totalsumsarray, function (n, catfield) {
                ...
                if (calculatedField != undefined) {
                    catfield.total = calculateValuesForCustomFieldsNumberChart(filteredData, catfield.lable, chartconfigobj.TableLocked)
                }
        ...
        for (var m = 0; m < serieslength; m++) {
            ...
            if (chartconfigobj.TableLocked && m > 5) {
...
function sortTable(column, type, tblIdentity, isThis, order) {
...
function AddRemoveClasses(element, SortTable) {
    ...
    if (isThis = false || $('#' + id).hasClass("sort-byAfter")) {
        if (SortTable == true) {
            ...
            sortTable(index, type, tblId, isThis, 'ASC');
        }
        else {
            sortTable(index, type, tblId, isThis, 'DESC');
        }
